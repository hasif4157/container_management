...
$sql_empno = mysqli_query($conn, "SELECT id FROM ex_staff order by id desc limit 1");
$num_empno = mysqli_num_rows($sql_empno);
$qry_empno = mysqli_fetch_array($sql_empno);

if ($num_empno > 0) {
    $str = $qry_empno['id'] + 1;
    $emp_no = "EMP" . str_pad($str, 5, "0", STR_PAD_LEFT);
} else {
    $str = 1;
    $emp_no = "EMP" . str_pad($str, 5, "0", STR_PAD_LEFT);
}
